...
## 8. 端到端 QA（LongMemEval-S 官方作答式）：检索 0.86 / 作答 0.28 ✅（口径落地，短板定位）
...
| 运行 | 样本 | 证据窗口 | 作答提示词 | evidence@1/3/5 | answer-substring@5 | answer-llm-judge@5 |
|---|---|---|---|---|---|---|
| r1 | 20 | 1200 字/会话 | 默认（有就答） | 0.80/0.85/0.85 | 0.10 | 0.10 |
| r2 | 20 | 3000 字/会话 | 默认 | 0.80/0.85/0.85 | 0.20 | 0.25 |
| r3 | 20 | 3000 字/会话 | 抽取式（少拒答） | 0.80/0.85/0.85 | 0.30 | 0.35 |
| **r4 全量** | **50** | 3000 字/会话 | 抽取式（少拒答） | **0.84/0.86/0.86** | **0.24** | **0.28** |

**失败分诊（r4 全量 50 题，skipped 0）**：retrieval-miss 7、answer-miss 29、ok 14；其中 43 题金标会话已进 top-5（占 86%）。三条可归因结论：

1. **检索层不是短板**（evidence@5 = 0.86，与 §7 的 0.86 一致）——真实对话式长记忆数据上混合检索够用；
2. **证据压实是短板之一**：LongMemEval 会话是多轮长对话，答案常在头部窗口之外（r1→r2 窗口翻 2.5 倍，作答从 0.10 → 0.25）；
3. **作答层（提示词/拒答倾向）是主要短板**：即使 43 题证据已命中，仅 14 题答对（命中后正确率 0.33）；抽取式提示词（r2→r3）能显著减少错拒答，但「整会话检索 + 头窗口截断 + 简单作答」组合下作答层仍是当前瓶颈。**后续方向**：检索改粗到细（先会话后定位关键段落）、作答改检索增强（RAG 定位）+ 更强抽取提示；同时写入端蒸馏补 topic 词（§4）可减少答案藏在长会话深处导致的漏召回。

**样本效应诚实口径**：20 题子集（r3）作答 0.35 高于全量 50 题（r4）0.28——前 20 题恰是较易样本，全量数字才是对外引用口径。

成本：50 题全量 ≈ 17.1 万 billed token（Flash，< $0.10）。

复现：`node evals/longmemeval-e2e.mjs --n 50 --fresh --json evals/results/longmemeval-e2e-50.json`（需 `DEEPSEEK_API_KEY`；不带 `--fresh` 用磁盘缓存续跑）
...
7. ~~端到端 QA 口径~~ **已完成**（v2.5.0：`evals/longmemeval-e2e.mjs` 作答+judge 分层；全量 50 题检索 0.86 / 作答 0.28，§8）。
